...
def query_all():
    ...
    for (i, row) in enumerate(rows):
        print(i,
              f"name: {row.username} phone: {row.phone} timezone: {row.timezone} timestamp: {row.timestamp} utc_start: {row.utc_start} utc_end: {row.utc_end}\n")
    db.close()
...
def formattedAvailabilityListToUTC(zones, available):
    ...
    for idx, i in enumerate(available):
        row = convertLocalStartToUtcStart(zones[idx], available[idx][0]), convertLocalStartToUtcStart(zones[idx],
                                                                                                      available[idx][1])
...
def call_duration_of_phone(phone):
    """
    This function takes input as a Phone Number and fetch the all the records of call
    durations for that number for the same day and return total numbers of seconds,
     person spends for the day.
    """
    table_name = 'call_logs2'  # name of the table from where are we fetching the call logs
    today = datetime.date.today()
    cur = conn.cursor()
    cur.execute(f"Select durations from {table_name} where phone = {phone} and date_time = '{today}'")
    rows = cur.fetchall()
    seconds = 0
    for row in rows:
        ...
        if len(duration) == 2:
            ...
            seconds += int(duration[4])
            seconds += (int(duration[2]) * 60)
            seconds += (int(duration[0]) * 3600)
        else:
            raise ValueError("Invalid format of the Duration")
    return seconds
